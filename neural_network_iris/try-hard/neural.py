...
class Neural:
    ...
    test_X = np.loadtxt("BezdekIris.txt", delimiter=",", usecols=(0, 1, 2, 3))
    test_X = nomalize(test_X)
    test_X = np.append(np.ones((test_X.shape[0], 1)), test_X, axis=1)

    def __init__(self, epsilon = 0.0001, numb_hidden_layer = 1, alpha = 0.1, iteration = 200, init_epsilon = 1, lambd = 0.1, hidden_layer_size = 4, class_size = 3):
    ...
    def backward(self):
        ...
        while epoch < self.iteration:
            ...
            # print self.cost_function(a3, W1, W2)
        return W1, W2
